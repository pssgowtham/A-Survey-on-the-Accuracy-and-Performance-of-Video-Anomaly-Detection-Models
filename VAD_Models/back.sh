...
# GitHub user info
GIT_AUTHOR_NAME="pssgowtham"
GIT_AUTHOR_EMAIL="gowthamsps98@gmail.com"
GIT_COMMITTER_NAME="pssgowtham"
GIT_COMMITTER_EMAIL="gowthamsps98@gmail.com"

# Loop through dates and create commits
for i in ${!dates[@]}; do
  echo "# Video Anomaly Detection Model ${i}" > model_${i}.py
  git add model_${i}.py
  GIT_AUTHOR_DATE="${dates[i]}T12:00:00" GIT_COMMITTER_DATE="${dates[i]}T12:00:00" git commit --date="${dates[i]}T12:00:00" -m "${messages[i]}"
done
